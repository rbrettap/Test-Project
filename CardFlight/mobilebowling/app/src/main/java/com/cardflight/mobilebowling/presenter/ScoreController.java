...
public class ScoreController {
    ...
    public  void startGame() {
        ...
        for(int i =0; i < 10; i++)  //for loop for 10 Frames
        {
            ...
            for(int j=0;j<=i;j++)
            {
                ...
                if((this.getGame().getFrames()[j].isStrike()==false)&& (this.getGame().getFrames()[j].isFrame_Spare()==false))
                {
                    //System.out.println("The Individual score for "+(j+1)+" Frame is :"+this.getGame().getFrames()[j].getAbsolute_score_of_frame());
                    System.out.println("The Individual score for "+(j+1)+" Frame is :"+ this.getGame().getFrames()[j].totalFrameScore);
                }
                //STRIKE not calculated yet
                if((this.getGame().getFrames()[j].isStrike()==true)&&(this.getGame().getFrames()[j].totalFrameScore ==-99))
                {
                ...
                if((this.getGame().getFrames()[j].isFrame_Spare()==true)&&(this.getGame().getFrames()[j].totalFrameScore ==-88))
                {
    ...
    public int getCumulativeScore(int i)
    {
        ...
        for(int j=0;j<=i;j++ )
        {
            if((this.getGame().getFrames()[j].totalFrameScore ==-99)||(this.getGame().getFrames()[j].totalFrameScore ==-88))
                sum=sum+0;
            else
                sum=sum+ this.getGame().getFrames()[j].totalFrameScore;
...

    {
        for(int i=j-1;i>=0;i--)
        {
            if((this.getGame().getFrames()[i].isFrame_Strike()==true) && (this.getGame().getFrames()[i].totalFrameScore ==-99)&&
            ...
            if((this.getGame().getFrames()[i].isStrike()==true) && (this.getGame().getFrames()[i].totalFrameScore ==-99)
                    && (this.getGame().getFrames()[i].isIs_first_updated()==true)&&(this.getGame().getFrames()[i].isIs_second_updated()==false) )
            {
                ...
                this.getGame().getFrames()[i].totalFrameScore;

                continue;
            }

            if((this.getGame().getFrames()[i].isSpare()==true) && (this.getGame().getFrames()[i].getTotal_score_of_this_frame()==-88)&&
